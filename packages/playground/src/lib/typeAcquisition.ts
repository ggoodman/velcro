import { join, PackageJson, resolve, Uri } from '@velcro/common';
import { Resolver } from '@velcro/resolver';
import { Emitter, Event } from 'ts-primitives';
import { EntryKind } from './hooks';
...
function getTypingsModuleSpec(pkgJson: {
  name: string;
  version?: string;
}): { name: string; range: string } {
  const typingsName =
    pkgJson.name.charAt(0) === '@' ? pkgJson.name.slice(1).replace('/', '__') : pkgJson.name;
...
export class TypeAcquirer {
  ...
  constructor(
    ...
    private readonly resolveBareModule: (spec: string, pathname?: string) => Uri
  ) {}
  ...
  private _importAdjacentTypings(
    ...
    const moduleRoot = join(parentPath, `node_modules/${pkgJson.name}/`);
    ...
    return this._listFiles(pkgJson.name, pkgJson.version).then((files) => {
      ...
      const normalizedMain = resolve('/', pkgJson.main || 'index.js');
      ...
      for (const pathname in files) {
        ...
        promises.push(
          this.resolver
            .readFileContent(this.resolveBareModule(`${pkgJson.name}@${pkgJson.version}`, pathname))
            .then(
              // eslint-disable-next-line no-loop-func
              (buf) => {
                const contents = this.resolver.decode(buf);
                const filePath = join(moduleRoot, pathname);
  ...
  private async _importTypingsFromDefinitelyTyped(
    ...
    const typingsModuleSpec = getTypingsModuleSpec({
      name: pkgJson.name,
      version: pkgJson.version,
    });

    try {
      const pkgJson = await this._import(
        files,
        typingsModuleSpec.name,
        typingsModuleSpec.range,
        parentPath
      );
  ...
  private async _listFiles(
    ...
    const unresolvedUrl = await this.resolver.getCanonicalUrl(
      this.resolveBareModule(`${name}@${version}`, '/')
    );
    const resolvedUrl = await this.resolver.getCanonicalUrl(unresolvedUrl.uri);
    const entries: { [pathname: string]: PackageFileEntry } = {};
    const queue = [resolvedUrl.uri];

    while (queue.length) {
      const next = queue.shift()!;
      const entriesAtPath = await this.resolver.listEntries(next);

      for (const entry of entriesAtPath.entries) {
        if (entry.type === EntryKind.Directory) {
          queue.push(entry.uri);
        } else {
          const path = entry.toString().slice(resolvedUrl.toString().length - 1);
  ...
  private async _import(
    ...
    const pkgJsonPathname = join(parentPath, 'node_modules', name, 'package.json');
    ...
    // } else {
    try {
      const hasAdjacentTypings = await this._importAdjacentTypings(
        files,
        resolvedPkgJson.packageJson!,
        parentPath
      );
      ...
      if (!hasAdjacentTypings && resolvedName.indexOf('@types/') !== 0) {
        await this._importTypingsFromDefinitelyTyped(
          files,
          resolvedPkgJson.packageJson,
          parentPath
        );
  ...
  async importTypesForSpec(
    name: string,
    range: string = 'latest',
    parentPath: string = ''
  ): Promise<TypingsFiles> {
  ...
  async importTypesForSpecs(
    specs: { [name: string]: string },
    parentPath: string = ''
  ): Promise<TypingsFiles> {
    const promises = Object.keys(specs).map((name) =>
      this.importTypesForSpec(name, specs[name], parentPath)
    );
