...
function tsc(dirname) {
  return {
    async buildStart() {
      ...
      await execFileAsync('npx', ['tsc', '--build', '.'], {
        cwd: dirname,
        timeout: 10000,
      });
    },
...
function rollupConfigFactory(dirname, packageJson) {
  ...
  return [
    ...
    // },
    {
      input: resolve(dirname, './build/index.js'),
      output: [
        {
          file: resolve(dirname, './dist/index.cjs.js'),
          format: 'cjs',
          sourcemap: true,
        },
        {
          file: resolve(dirname, './dist/index.esm.js'),
          format: 'esm',
          sourcemap: true,
        },
      ],
